...
# 复制 rules.txt 和 allow.txt 到 history 目录  
cp ./rules.txt ./history/  
cp ./allow.txt ./history/  
  
# 压缩 rules.txt 和 allow.txt 为 zip 格式，并使用当前时间+原文件名作为新文件名  
zip -r ./history/$(date +'%Y%m%d%H%M%S')_rules.zip ./rules.txt  
zip -r ./history/$(date +'%Y%m%d%H%M%S')_allow.zip ./allow.txt  
  
# 删除被压缩的 rules.txt 和 allow.txt 文件  
rm -f ./history/rules.txt  
rm -f ./history/allow.txt
